...
// 6) -------------------------------------------------------
function nested(numbers) {  // space n
    for (let i = 0; i < numbers.length; i++) {  // space 1
      for (let j = 0; j < numbers.length; j++) {  // space 1
        console.log(j)
      }
    }
  }
...
// ANSWER: No. Possible to reduce storage but not complexity.
...
function countLetters(str) {  // O(n)
  ...
  str = str.toLowerCase()  // 'abc', same storage
  const splitStr = str.split('')  // ['a','b','c'], n new storage
...
function countLettersRoundTwo(str) { // n
  // this contains a RegEx or Regular Expression, feel free to google
  str = str.toLowerCase().replace(/([^a-z])+/g,'')
  let letters = {} // 1
  for (let i = 0; i < str.length; i++) { // 1 for the i var
